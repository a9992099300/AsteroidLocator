...
internal data class NeoFeedDto(
...
data class NearEarthObjectDto(
    ...
    @Json(name = "estimated_diameter") val estimatedDiameter: NeoEstimatedDiameterDto? = null,//+
    @Json(name = "is_potentially_hazardous_asteroid") val isPotentiallyHazardousAsteroid: Boolean? = null,//+
    @Json(name = "close_approach_data") val closeApproachData: List<NeoCloseApproachDataDto>? = null,//+
    @Json(name = "orbital_data") val orbitalData: NeoOrbitalDataDto? = null,//-
)

@JsonClass(generateAdapter = true)
data class NeoEstimatedDiameterDto(
    @Json(name = "meters") val meters: NeoDiameterRangeDto? = null//+
)
