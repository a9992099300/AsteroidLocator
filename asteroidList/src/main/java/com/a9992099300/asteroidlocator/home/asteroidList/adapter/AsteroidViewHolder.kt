...

{
    ...
    fun bind(asteroid: NearEarthObjectUI, position: Int) {
        ...
        if (asteroid.isPotentiallyHazardousAsteroid) binding.asteroidDangerousImageView.imageTintList =
            context.resources.getColorStateList(R.color.orange)

        if (asteroid.isFavorite) binding.asteroidFavoriteImageView.imageTintList =
            context.resources.getColorStateList(R.color.yellow)
